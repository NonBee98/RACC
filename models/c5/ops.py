...
def get_uv_coord(hist_size, tensor=True, normalize=False, device='cuda'):
  ...
  """
    ...
    if tensor:
        u_coord = torch.from_numpy(u_coord).to(device=device,
                                               dtype=torch.float32)
        u_coord = torch.unsqueeze(u_coord, dim=0)
        u_coord.requires_grad = False
        v_coord = torch.from_numpy(v_coord).to(device=device,
                                               dtype=torch.float32)
...
def complex_multiplication(a, b):
  ...
  """
    ...
    imag_b = b[:, :, :, :, 1]
    result = torch.stack(
        [real_a * real_b - imag_a * imag_b, real_a * imag_b + imag_a * real_b],
        dim=-1)
    return result
...
def compute_histogram(chroma_input, hist_boundary, nbins, rgb_input=None):
  ...
  """
    ...
    A_u = np.arange(hist_boundary[0], hist_boundary[1] + eps / 2,
                    eps)  # -2.86 to 2.86
    A_v = np.flip(A_u)
    ...
    diff_u = np.abs(
        np.tile(chroma_input[:, 0], (len(A_u), 1)).transpose() -
        np.tile(A_u, (len(chroma_input[:, 0]), 1)))

    # differences in log_V space
    diff_v = np.abs(
        np.tile(chroma_input[:, 1], (len(A_v), 1)).transpose() -
...
def vis_tensor(tensor, norm=True, dim=None, scale=1):
  ...
  """
    ...
    if norm:
        tensor = (tensor - torch.min(tensor)) / (torch.max(tensor) -
                                                 torch.min(tensor))
    tensor = tensor * scale
    return tensor
...
def uv_to_rgb(uv, tensor=False):
  ...
  """
    ...
    if tensor:
        ...
        rgb = torch.stack([
            rb[:, 0],
            torch.ones(rb.shape[0], dtype=uv.dtype, device=uv.device), rb[:, 1]
        ],
                          dim=-1)
...
def compute_edges(im):
  ...
  """
    ...
    offsets = [-1, 0, 1]
    for filter_index, (dx,
                       dy) in enumerate(itertools.product(offsets, repeat=2)):
        if dx == 0 and dy == 0:
            continue
        edge_img[:, :, :] = edge_img[:, :, :] + (np.abs(im[:, :, :] - img_pad[
            1 + dx:im.shape[0] + 1 + dx, 1 + dy:im.shape[1] + 1 + dy, :]))
    edge_img = edge_img / 8
    return edge_img
    ...
    for camera_model in cameras:
        ...
        for filename in filenames:
            ...
            if ext.lower() in img_extensions:
                print(
                    os.path.join(dataset_dir_new,
                                 f'{base}_sensorname_{postfix}.png'))
                copyfile(
                    filename,
                    os.path.join(dataset_dir_new,
                                 f'{base}_sensorname_{postfix}.png'))
                metadata_file = os.path.join(camera_model,
                                             base + '_metadata.json')
                ...
                copyfile(
                    metadata_file,
                    os.path.join(dataset_dir_new,
                                 f'{base}_sensorname_{postfix}_metadata.json'))
