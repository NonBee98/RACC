...
from utils import angular_error_torch

__all__ = ['C5', 'c5_loss']


def c5_loss(outputs, target: torch.Tensor) -> torch.Tensor:
    smoothness_factor_F = 0.15
    smoothness_factor_B = 0.02
    smoothness_factor_G = 0.02
    rgb, P, F, B, G = outputs
    u_variation = [[1, 0, -1], [2, 0, -2], [1, 0, -1]]
    u_variation = torch.tensor(u_variation,
                               dtype=target.dtype).unsqueeze(0).expand(
                                   1, 1, 3, 3).to(device=target.device)

    v_variation = [[1, 2, 1], [0, 0, 0], [-1, -2, -1]]
    v_variation = torch.tensor(v_variation,
                               dtype=target.dtype).unsqueeze(0).expand(
                                   1, 1, 3, 3).to(device=target.device)
    ae = angular_error_torch(rgb, target)
    if len(B.shape) == 2:
    ...
    s_loss_B = smoothness_factor_B * (torch.mean(
        torch.nn.functional.conv2d(
            torch.unsqueeze(B, dim=1), u_variation, stride=1)**2) + torch.mean(
                torch.nn.functional.conv2d(
                    torch.unsqueeze(B, dim=1), v_variation, stride=1)**2))

    # smoothing regularization for G (if applied)
    if G is not None:
        s_loss_G = smoothness_factor_G * (torch.mean(
            torch.nn.functional.conv2d(
                torch.unsqueeze(G, dim=1), u_variation, stride=1)**
            2) + torch.mean(
                torch.nn.functional.conv2d(
                    torch.unsqueeze(G, dim=1), v_variation, stride=1)**2))
    else:
        s_loss_G = 0
    ...
    s_loss_F_chroma = (
        ...
        torch.mean(
            torch.nn.functional.conv2d(
                torch.unsqueeze(F_chroma, dim=1), v_variation, stride=1)**2))
    ...
    s_loss_F_edges = (
        ...
        torch.mean(
            torch.nn.functional.conv2d(
                torch.unsqueeze(F_edges, dim=1), v_variation, stride=1)**2))

    s_loss_F = smoothness_factor_F * (s_loss_F_chroma + s_loss_F_edges)

    # final smoothing regularization
    smoothness_loss = s_loss_F + s_loss_G + s_loss_B
    loss = ae + smoothness_loss
    return loss


class C5(nn.Module):

    def __init__(self, input_size=64, learn_g=False, data_num=1, device='cpu'):
    ...
    """
        ...
        assert (input_size - 2**math.ceil(math.log2(input_size)) == 0
                and input_size >= 16)
        assert (data_num >= 1)
        ...
        self.u_coord, self.v_coord = get_uv_coord(self.input_size,
                                                  tensor=True,
                                                  device=self.device)
        ...
        intermediate_sz = pow(2,
                              math.ceil(math.log2(input_size)) - network_depth)
        assert (max_conv_depth > initial_conv_depth * 2)
        network_depth = int(
            math.ceil(math.log2(input_size)) - math.log2(intermediate_sz))

        # Encoder-decoder C5 net
        # encoder
        self.encoder = Encoder(in_channel=4,
                               first_conv_depth=initial_conv_depth,
        ...
        self.bottleneck = DoubleConvBlock(
            in_depth=min(initial_conv_depth * 2**(network_depth - 1),
                         max_conv_depth),
            mid_depth=min(initial_conv_depth * 2**network_depth,
                          max_conv_depth),
            out_depth=min(initial_conv_depth * 2**(network_depth - 1),
                          max_conv_depth),
            pooling=False,
    ...
    def forward(self, inputs):
        if isinstance(inputs, dict):
            ...
            N = inputs
        self.u_coord = self.u_coord.to(N.device)
        self.v_coord = self.v_coord.to(N.device)

        model_in_N = torch.unsqueeze(N, 1)
        ...
        P = self.softmax(
            torch.reshape(N_after_bias, (N_after_bias.shape[0], -1)))
        ...
        u, v = from_coord_to_uv(self.input_size, u, v)
        rgb = uv_to_rgb(torch.stack([u, v], dim=1), tensor=True)
        return (rgb, P, F, B, G)
...
class DoubleConvBlock(nn.Module):
    ...
    """
        ...
        if normalization:
            ...
            norm = [False, False]
        self.double_conv_1 = ConvBlock(kernel=kernel,
                                       in_depth=in_depth,
                                       conv_depth=mid_depth,
                                       stride=stride,
                                       padding=padding[0],
                                       pooling=False,
                                       dilation=dilation[0],
    ...
    """
        ...
        for data_i in range(self.data_num):
            ...
            for block_j in range(self.encoder_depth):
                ...
                out_depth = min(first_conv_depth * (2**block_j),
                                max_conv_depth)
                ...
                double_conv_block = DoubleConvBlock(
                    ...
                    norm_type=norm_type,
                    normalization_block='Second',
                    return_before_pooling=skip_connections)
        ...
        for encoder_block_i in range(self.encoder_depth):
            for data_j in range(self.data_num):
                if encoder_block_i == 0:
                    if data_j == 0:
                        curr_latent, latent_before_pooling = self.encoders[
                            data_j][encoder_block_i](x[:, data_j, :, :, :])
                    ...
                    if data_j == 0:
                        curr_latent, latent_before_pooling = self.encoders[
                            data_j][encoder_block_i](latent_x[data_j])
                        skip_connection_data.append(latent_before_pooling)
                    else:
                        curr_latent = self.encoders[data_j][encoder_block_i](
                            latent_x[data_j])
                ...
                if self.merge_layers is not None:
                    if data_j == 0:
                        stacked_latent = torch.unsqueeze(latent_x[data_j],
                                                         dim=-1)
                    else:
                        stacked_latent = torch.cat([
                            stacked_latent,
                            torch.unsqueeze(latent_x[data_j], dim=-1)
                        ],
                                                   dim=-1)
            if self.merge_layers is not None:
                ...
                if encoder_block_i < (self.encoder_depth - 1):
                    for data_j in range(self.data_num):
                        latent_x[data_j] = self.merge_layers[data_j][
                            encoder_block_i](torch.cat(
                                [latent_x[data_j], pooled_data], dim=1))
...
class Decoder(nn.Module):
    ...
    def __init__(self,
                 output_channels,
                 encoder_first_conv_depth=8,
                 normalization=False,
                 encoder_max_conv_depth=32,
                 norm_type='IN',
                 depth=4):
        """ Decoder constructor
    ...
    """
        ...
        self.upsampling = nn.Upsample(scale_factor=2,
                                      mode='bilinear',
                                      align_corners=True)
        self.final_block = ConvBlock(kernel=3,
                                     activation=False,
                                     in_depth=int(encoder_first_conv_depth /
                                                  2),
                                     conv_depth=output_channels,
                                     stride=1,
                                     padding=1)
        for i in range(depth):
            mid_depth = int(
                min(encoder_first_conv_depth * 2**(depth - 1 - i),
                    encoder_max_conv_depth))
            out_depth = int(
                min(encoder_first_conv_depth * 2**(depth - 2 - i),
                    encoder_max_conv_depth))
            in_depth = int(
                min(encoder_first_conv_depth * 2**(depth - i),
                    encoder_max_conv_depth * 2))
            double_conv_block = DoubleConvBlock(in_depth=in_depth,
                                                out_depth=out_depth,
                                                mid_depth=mid_depth,
                                                normalization_block='Second',
