...
class MLP(nn.Module):
    ...
    def __init__(self,
        ...
        self.first_layer = nn.Sequential(nn.Linear(in_feature, neurons),
                                         nn.LeakyReLU())
        self.hidden_layer = nn.ModuleList([
            nn.Sequential(nn.Linear(neurons, neurons), nn.LeakyReLU())
...
class AdaptiveInstanceNorm2d(nn.Module):
    ...
    def __init__(self, style_dim, num_features, eps=1e-9):
        super(AdaptiveInstanceNorm2d, self).__init__()
        self.num_features = num_features
        self.style_dim = style_dim
        self.eps = eps

        self.gamma_transform = nn.Linear(style_dim, num_features, bias=False)
        self.beta_transform = nn.Linear(style_dim, num_features, bias=False)

    def forward(self, x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
        # x: (N, C, H, W)
        # w: (N, style_dim)
        gamma: torch.Tensor = self.gamma_transform(y)  # (N, num_features)
        beta: torch.Tensor = self.beta_transform(y)  # (N, num_features)

        mean = x.mean([2, 3], keepdim=True)
        var = (x - mean).pow(2).mean([2, 3], keepdim=True)
        out = (x - mean) / (var + self.eps).sqrt()  # (N, C, H, W)
        out = out * gamma.reshape([gamma.shape[0], gamma.shape[1], 1, 1
                                   ]) + beta.reshape(
...
class Conv(nn.Module):
    ...
    def forward(self, x):
        return self.dropout(self.convs(x))
    ...
    def fuse_params(self):
        if (not self.quant) and hasattr(
                self.convs,
                'conv') and self.norm == 'bn':
            convs = fuse_conv_bn(
                self.convs._modules["conv"], self.convs._modules["norm"]
            )
            act = self.get_activation()
            self.convs = nn.Sequential()
    ...
    def get_fused_kernel(self):
        if (
            (not self.quant)
            and hasattr(self.convs, "conv")
            and hasattr(self.convs, "norm")
            and self.norm == "bn"
        ):
            convs = fuse_conv_bn(
                self.convs._modules["conv"], self.convs._modules["norm"]
