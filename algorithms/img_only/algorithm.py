...
def _extract_features(inputs: dict) -> torch.Tensor:
    ...
    edge_img = (
        kornia.filters.sobel(img.permute(2, 0, 1).unsqueeze(0))
        .squeeze()
        .permute(1, 2, 0)
    )
...
def img_only(inputs: dict, **kwargs) -> np.ndarray:
    global model
    ...
    if opt.ckpt is not None:
        ...
        model_path = os.path.join(opt.checkpoint_path, opt.model_name, "best.ckpt")
    if model is None:
        model = ImgOnly()
